...
const MultiStep = (props) => {
  ...
  const [buttonsState, setButtons] = useState(getButtonsState(0, props.steps.length,props.showNext, props.showPrev))

  const setStepState = (indx) => {
    setStyles(getNavStyles(indx, props.steps.length))
    setComp(indx < props.steps.length ? indx : compState)
    setButtons(getButtonsState(indx, props.steps.length,props.showNext, props.showPrev))
  }
